...
class CNNModelV2_paddleLeft(TorchModelV2, torch.nn.Module):
...
class CNNModelV2_paddleRight(TorchModelV2, torch.nn.Module):
    def __init__(self, obs_space, act_space, num_outputs, *args, **kwargs):
        TorchModelV2.__init__(self, obs_space, act_space, num_outputs, *args, **kwargs)
        torch.nn.Module.__init__(self)
        # self.model = torch.nn.Sequential(
        ...
        # self.policy_fn = torch.nn.Linear(512, num_outputs)
        # self.value_fn = torch.nn.Linear(512, 1)
                # Assuming obs_space is a Box space
        self.observation_space = obs_space

        # Assuming action_space is a Discrete space
        self.action_space = act_space

        # Define a constant output for the "do nothing" action
        self.constant_output = torch.zeros(num_outputs)

    def forward(self, input_dict, state, seq_lens):
        return self.constant_output, state
        action_logits = torch.zeros((len(input_dict["obs"]), self.action_space.n))
