...
from Enviroment.cooperative_pong.ball import Ball
from Enviroment.cooperative_pong.cake_paddle import CakePaddle
from Enviroment.cooperative_pong.manual_policy import ManualPolicy
from Enviroment.cooperative_pong.paddle import Paddle
from Enviroment.cooperative_pong.paddle_train import Paddle_Train
    ...
    def step(self, action, agent):
        ...
        if agent == self.agents[0]:
            ...
            if not self.terminate:
                ...
                # sprawdzenie czy piłka jest odbita
                p0_collision,_, _ = self.p0.process_collision(self.ball.rect,self.ball.speed,1)
                p1_collision,_, _ = self.p1.process_collision(self.ball.rect,self.ball.speed,2)
                # do the miscellaneous stuff after the last agent has moved
                # reward is the length of time ball is in play

                reward = 0
                hit_reward = 1 # reward given to agent on hit
                ...
                if not self.terminate:
                    self.num_frames += 1
                    reward = self.max_reward / self.max_cycles *  (1 + 0.01 * self.num_frames)
                    self.score += reward
                ...
                for ag in self.agents:
                    ...
                    if ag == self.agents[0] and  self.ball.agent_colision[0]:
                        self.rewards[ag] += hit_reward
                    if ag == self.agents[1] and  self.ball.agent_colision[1]:
                        self.rewards[ag] += hit_reward
